...
1.4. Тест-сьют: Отправка формы записи на обучение профессии "Тестировщик ПО"

Тест-кейсы:

Отправка пустой формы:
1) ничего не заполняя кликнуть на кнопку "Записаться". Ожидаемый результат: границы полей "Имя" и "Телефон" выделяются красным цветом и под каждым полем появляется ошибка с текстом "Обязательное поле"

Отправка заполненной формы валидными данными (предусловие: а) для поля "Имя" валидные данные-только буквы, на кириллице и/или латинице, а также пробел или дефис между букв, регистр значения не имеет, ограничение на кол-во букв: минимум 2, максимум 255; б) для поля "Телефон" валидные данные-только арабские цифры, ограничение на кол-во цифр: минимум 9, максимум 14):
1) ввести в поле "Имя" валидные данные, например, "Ольга".
Ожидаемый результат: поле заполнено корректно
2) ввести в поле "Телефон" валидные данные, например, "89657893611".
Ожидаемый результат: поле заполнено корректно
3) кликнуть на кнопку "Записаться".
Ожидаемый результат: запись прошла успешно, появляется сообщение "Менеджер свяжется с Вами в ближайшее время"
